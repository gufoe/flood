...
(function() {
	...
	this.finalize = () => {
	    this.fitness+= 1*Object.keys(_explored).length
	}
        ...
        this.update = () => {
            ...
            for (var y = _player.y - 1; y <= _player.y + 1; y++) {
                for (var x = _player.x - 1; x <= _player.x + 1; x++) {
                    ...
                    i_main.push(_world.isWall(x, y) ? 1 : 0)
                    i_main.push(_world.search(x, y, Dna.c.code) ? 1 : 0)
                    // i_main.push(o_type[0])
                }
            }
...
*/
    ...
    Dna.gen = () => {
        ...
        // Cells output + round
        dna.n_main = new Net([9*2+1, 2, 3])

        // Inputs: [wall, code]
        dna.n_cell = new Net([2, 5, 10, 1])

        // Inputs: [empty, inaccessible, self, others, code, enemy, weapon]
        dna.n_type = new Net([4, 5, 10, 1])
